...
from .config import STREAMLIT_MODEL_ARTIFACTS_DIR

INPUT_VARS = ["PRECTmms", "TBOT", "WIND", "QBOT", "PSRF", "FSDS", "FLDS"]
DEFAULT_MODEL_GRID_PATH = os.path.join(STREAMLIT_MODEL_ARTIFACTS_DIR, "grid", "model_grid.npz")
...
def _compute_nldas_crop_slices(
    sample_forcing_file: str, latmin: float, latmax: float, lonmin: float, lonmax: float, pad_cells: int = 1
):
...
def load_local_daily_forcing(
    ...
    if not files:
        raise FileNotFoundError(
            f"No forcing files found for requested years: {years}. Directory checked: {forcing_dir}"
        )
